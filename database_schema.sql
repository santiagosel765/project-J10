...
-- This version uses unquoted table names in DROP statements to avoid case-sensitivity issues.
...
-- Users Table
-- Note: "hashedPassword" is quoted to preserve the camelCase, as it is used this way in the application code.
CREATE TABLE Users (
    ...
    image TEXT, -- Using TEXT for Data URI
...
-- Campaigns Table
-- ON DELETE RESTRICT prevents deleting a template if it's used by a campaign.
...
-- When a template is deleted, the rule is not deleted but template_id is set to NULL.
